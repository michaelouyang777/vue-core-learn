...
export function renderMixin (Vue: Class<Component>) {
  ...
  Vue.prototype._render = function (): VNode {
    ...
    // 从$options中取出render函数和parentVnode
    ...
    try {
      ...
      // 不需要维护堆栈，因为所有render函数都是单独调用的。
      // Nested component's render fns are called when parent component is patched.
      // 在修补父组件时，将调用嵌套组件的render函数。
      currentRenderingInstance = vm
      // 使用call方法， 将this指向 vm.renderProxy，vm.renderProxy是个代理，代理vm，主要用来报错，如果render函数上使用了vm上没有的属性或方法，就会报错。
      // vm.$createElement 这个是创建vnode的方法，作为render的第一个参数传入
    ...
    // 如果vnode不是VNode的实例，则返回一个空的VNode节点
